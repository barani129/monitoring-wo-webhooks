...
func (r *PortScanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	runningHost, err = clusterUtil.GetAPIName(*clientset)
	if err != nil || runningHost == "" {
		log.Log.Info("unable to retrieve domain name")
		runningHost = "hub-cluster"
	}
	...
	if clusterStatus.LastPollTime == nil {
		...
		for _, target := range clusterSpec.Target {
			...
			if err != nil {
				log.Log.Info(fmt.Sprintf("target %s is unreachable with err %s", clusterSpec.Target, err.Error()))
				...
				if !slices.Contains(clusterStatus.AffectedTargets, target) {
					...
					if clusterSpec.SuspendEmailAlert != nil && !*clusterSpec.SuspendEmailAlert {
						clusterUtil.SendEmailAlert(runningHost, fmt.Sprintf("/home/golanguser/files/port/.%s-%s.txt", clusterUtil.HandleCNString(ip[0]), ip[1]), clusterSpec, fmt.Sprintf("Target %s is not reachable on port %s from k8s cluster %s", ip[0], ip[1], runningHost))
					}
					if clusterSpec.NotifyExtenal != nil && *clusterSpec.NotifyExtenal {
						err := clusterUtil.NotifyExternalSystem(data, "firing", target, clusterSpec.ExternalURL, string(username), string(password), fmt.Sprintf("/home/golanguser/files/port/.%s-%s-ext.txt", clusterUtil.HandleCNString(ip[0]), ip[1]), clusterStatus)
						if err != nil {
							log.Log.Error(err, "Failed to notify the external system")
						}
						fingerprint, err := clusterUtil.ReadFile(fmt.Sprintf("/home/golanguser/files/port/.%s-%s-ext.txt", clusterUtil.HandleCNString(ip[0]), ip[1]))
						if err != nil {
				...
				if _, err := os.Stat(fmt.Sprintf("/home/golanguser/files/port/.%s-%s.txt", clusterUtil.HandleCNString(ip[0]), ip[1])); os.IsNotExist(err) {
					// no action
				} else {
					if slices.Contains(clusterStatus.AffectedTargets, target) {
						if clusterSpec.SuspendEmailAlert != nil && !*clusterSpec.SuspendEmailAlert {
							clusterUtil.SendEmailRecoveredAlert(runningHost, fmt.Sprintf("/home/golanguser/files/port/.%s-%s.txt", clusterUtil.HandleCNString(ip[0]), ip[1]), clusterSpec, fmt.Sprintf("Target %s is now reachable again on port %s from k8s cluster %s", ip[0], ip[1], runningHost))
						}
						if clusterSpec.NotifyExtenal != nil && *clusterSpec.NotifyExtenal && clusterStatus.ExternalNotified {
							...
							err = clusterUtil.SubNotifyExternalSystem(data, "resolved", target, clusterSpec.ExternalURL, string(username), string(password), fmt.Sprintf("/home/golanguser/files/port/.%s-%s.txt", clusterUtil.HandleCNString(ip[0]), ip[1]), clusterStatus)
							if err != nil {
								log.Log.Error(err, "Failed to notify the external system")
							}
							now := metav1.Now()
		...
		if len(errorp) > 0 {
			...
			return ctrl.Result{RequeueAfter: defaultHealthCheckIntervalPort}, fmt.Errorf("%s", "one of the target or all are unreachable")
		}
			...
			for _, target := range clusterSpec.Target {
				...
				if err == nil {
					...
					if !slices.Contains(clusterStatus.AffectedTargets, target) {
						...
						if clusterSpec.NotifyExtenal != nil && *clusterSpec.NotifyExtenal {
							err := clusterUtil.SubNotifyExternalSystem(data, "firing", target, clusterSpec.ExternalURL, string(username), string(password), fmt.Sprintf("/home/golanguser/files/port/.%s-%s-ext.txt", clusterUtil.HandleCNString(ip[0]), ip[1]), clusterStatus)
							if err != nil {
								log.Log.Error(err, "Failed to notify the external system")
							}
							fingerprint, err := clusterUtil.ReadFile(fmt.Sprintf("/home/golanguser/files/port/.%s-%s-ext.txt", clusterUtil.HandleCNString(ip[0]), ip[1]))
